<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($save)) {
            ...
            if (empty($CID) || empty($CName) || empty($CourseCode) || empty($CSections) ||  empty($Time) || empty($ExamDate) ||   empty($ExamTime) || empty($ExamRooms)  || empty($Instructors) ||empty($ETime) ) 
            {
                $error = true;
            } else {
                try {
                    ...
                    $db->beginTransaction();
                       ...
                       $stmt = $db->prepare(" insert into courses (CourseID, CourseName, Code, CCredits, Time,ETime, Exame_Date, 
                       Instructors, CSections, Rooms, ExamTime, ExamRooms, Department,
                        DepNumber, Description) 
                        VALUES (:CourseID, :CourseName, :Code, :CCredits, :Time,:ETime,:ExamDate, :Instructors, :CSections, :Rooms, :ExamTime, :ExamRooms, :Department, :DepNumber, :Description)");
                        ...
                        $stmt->execute();
                             ...
                             $stmts = $db->prepare("insert into section ( Seats, CName, LactureTime,ETime ,Instructor, CCode,CourseID) 
                                VALUES ( :Seats, :CName, :LectureTime, :ETime,:Instructors, :CCode,:CourseID)");
...
if (isset($delete)) {
  try {
      ...
      $stmt = $db->prepare("DELETE FROM courses WHERE CourseID = :CourseID");
      $stmt->bindParam(':CourseID', $DelteCID);
      $stmt->execute();

      // Delete the course from the section table
      $stmts = $db->prepare("DELETE FROM section WHERE CourseID = :CourseID");
      $stmts->bindParam(':CourseID', $DelteCID);
      $stmts->execute();
...
<!-- start main  -->
  <main>
        ...
        <div class="m-5 p-5 border rounded">
          <p style=" color:#25364b; font-size:30px; ">Add Courses</p>
            ...
            <div class="col">
                ...
                <span>From Time :</span>
                    <div class="input-group mb-3">
                        <span class="input-group-text" id="basic-addon1">Start lecture Time  </span>
    ...
    <section id="form">
        <div class="m-5 p-5 border rounded">
        <p style=" color:#25364b; font-size:30px; ">Delete Courses</p>
        ...
        <div class="row">
                <div class="col">
                    ...
                    <div class="input-group mb-3">
                        <span class="input-group-text" id="basic-addon1">ID</span>
                        <input name="DelteCID" type="text" class="form-control" placeholder="CourseName#Section" aria-label="Username"
                            aria-describedby="basic-addon1">
                    </div>
                </div>
                <div class="col">
